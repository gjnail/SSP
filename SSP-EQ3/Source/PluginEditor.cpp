...
PluginEditor::PluginEditor(PluginProcessor& p)
    : AudioProcessorEditor(&p),
      ...
      controls(p.apvts)
{
    setLookAndFeel(&eq3ui::getVectorLookAndFeel());
...
void PluginEditor::paint(juce::Graphics& g)
{
    ...
    auto divider = header.withTrimmedLeft(166.0f).withTrimmedRight(16.0f).withHeight(1.0f).withCentre({ header.getCentreX(), header.getBottom() - 1.0f });
...
void PluginEditor::resized()
{
    ...
    auto titleArea = header.removeFromLeft(280);
