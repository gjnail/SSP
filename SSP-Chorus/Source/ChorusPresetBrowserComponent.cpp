...
ChorusPresetBrowserComponent::ChorusPresetBrowserComponent(PluginProcessor& p)
    : processor(p)
{
...
void ChorusPresetBrowserComponent::setAnchorBounds(juce::Rectangle<int> button, juce::Rectangle<int> graph)
{
    anchorBounds = button;
    graphBounds = graph;
    updateMenuLayout();
...
void ChorusPresetBrowserComponent::open()
{
...
void ChorusPresetBrowserComponent::close()
{
    endPreviewIfNeeded();
    hideDialog();
...
bool ChorusPresetBrowserComponent::handleKeyPress(const juce::KeyPress& key)
{
...
void ChorusPresetBrowserComponent::paint(juce::Graphics& g)
{
    if (! visibleOrAnimating)
        return;
    ...
    for (const auto& panel : panels)
    {
        ...
        reverbui::drawPanelBackground(g, panelBounds, accentColour(), 16.0f);
    ...
    if (dialogMode != DialogMode::none)
    {
        ...
        g.setOpacity(openProgress);
        reverbui::drawPanelBackground(g, dialogBounds, accentColour(), 18.0f);
    }
}

void ChorusPresetBrowserComponent::resized()
{
    updateMenuLayout();
    updateDialogLayout();
...
void ChorusPresetBrowserComponent::mouseMove(const juce::MouseEvent& event)
{
    if (dialogMode != DialogMode::none)
        return;
...
void ChorusPresetBrowserComponent::mouseExit(const juce::MouseEvent&)
{
    clearHoverPreviewPending();
    endPreviewIfNeeded();
}

void ChorusPresetBrowserComponent::mouseDown(const juce::MouseEvent& event)
{
    if (dialogMode != DialogMode::none)
    {
        auto dialogBounds = juce::Rectangle<float>(dialogWidth, dialogMode == DialogMode::deleteUserPreset ? 340.0f : dialogHeight)
...
void ChorusPresetBrowserComponent::mouseWheelMove(const juce::MouseEvent& event, const juce::MouseWheelDetails& wheel)
{
...
int ChorusPresetBrowserComponent::getNumRows()
{
    return deletableUserPresets.size();
}

void ChorusPresetBrowserComponent::paintListBoxItem(int rowNumber, juce::Graphics& g, int width, int height, bool rowIsSelected)
{
...
void ChorusPresetBrowserComponent::listBoxItemClicked(int row, const juce::MouseEvent& event)
{
    if (event.x >= deleteListBox.getWidth() - 84)
        deletePresetAtRow(row);
}

void ChorusPresetBrowserComponent::timerCallback()
{
    if (! visibleOrAnimating)
    {
        stopTimer();
...
void ChorusPresetBrowserComponent::rebuildMenu()
{
    panels.clear();
    MenuPanelState root;
...
std::vector<ChorusPresetBrowserComponent::MenuItem> ChorusPresetBrowserComponent::buildRootMenuItems() const
{
...
std::vector<ChorusPresetBrowserComponent::MenuItem> ChorusPresetBrowserComponent::buildCategoryItems(const juce::Array<PluginProcessor::PresetRecord>& presets, int depth) const
{
    std::vector<MenuItem> items;
...
void ChorusPresetBrowserComponent::updateMenuLayout()
{
    if (panels.empty())
        return;
...
void ChorusPresetBrowserComponent::updateDialogLayout()
{
...
void ChorusPresetBrowserComponent::setHighlightedItem(int panelIndex, int itemIndex, bool openChildren)
{
    ...
    if (openChildren && juce::isPositiveAndBelow(itemIndex, (int) panel.items.size()))
    {
...
void ChorusPresetBrowserComponent::moveHighlight(int delta)
{
    if (panels.empty())
        return;
...
void ChorusPresetBrowserComponent::openHighlightedSubmenu()
{
    if (panels.empty())
        return;
...
void ChorusPresetBrowserComponent::closeLastSubmenu()
{
    if (panels.size() > 1)
    {
        panels.pop_back();
...
void ChorusPresetBrowserComponent::activateHighlightedItem()
{
    if (panels.empty())
        return;
...
int ChorusPresetBrowserComponent::findNextSelectableIndex(const MenuPanelState& panel, int startIndex, int direction) const
{
...
float ChorusPresetBrowserComponent::getItemHeight(const MenuItem& item) const
{
    return item.kind == MenuItem::Kind::divider ? dividerHeight : rowHeight;
}

float ChorusPresetBrowserComponent::getPanelContentHeight(const MenuPanelState& panel) const
{
    float total = 0.0f;
...
juce::Rectangle<float> ChorusPresetBrowserComponent::getPanelContentBounds(const MenuPanelState& panel) const
{
    return panel.bounds.reduced(10.0f, 10.0f);
}

juce::Rectangle<float> ChorusPresetBrowserComponent::getItemBounds(const MenuPanelState& panel, int itemIndex) const
{
...
bool ChorusPresetBrowserComponent::isPointInsideAnyPanel(juce::Point<float> point) const
{
    for (const auto& panel : panels)
...
std::pair<int, int> ChorusPresetBrowserComponent::hitTestItem(juce::Point<float> point) const
{
...
void ChorusPresetBrowserComponent::updateHoverState(juce::Point<float> point)
{
...
void ChorusPresetBrowserComponent::clearHoverPreviewPending()
{
    pendingPreviewPresetKey.clear();
    pendingPreviewDeadline = 0;
}

void ChorusPresetBrowserComponent::maybeBeginHoverPreview()
{
...
void ChorusPresetBrowserComponent::endPreviewIfNeeded()
{
    processor.endPresetPreview();
}

void ChorusPresetBrowserComponent::executeMenuItem(const MenuItem& item)
{
    if (! item.enabled)
        return;
...
void ChorusPresetBrowserComponent::executeAction(const juce::String& actionID)
{
    if (actionID == "save-as")
    {
        showSaveDialog();
...
void ChorusPresetBrowserComponent::loadPreset(const PluginProcessor::PresetRecord& preset)
{
    endPreviewIfNeeded();
...
void ChorusPresetBrowserComponent::showSaveDialog()
{
...
void ChorusPresetBrowserComponent::showDeleteDialog()
{
...
void ChorusPresetBrowserComponent::showResetDialog()
{
    dialogMode = DialogMode::confirmReset;
    dialogTitleLabel.setText("Reset User Presets", juce::dontSendNotification);
    dialogMessageLabel.setText("This removes all user-saved SSP Chorus presets from your local library. Factory presets stay intact.", juce::dontSendNotification);
...
void ChorusPresetBrowserComponent::hideDialog()
{
...
void ChorusPresetBrowserComponent::confirmSaveDialog()
{
    if (dialogMode != DialogMode::saveAs)
        return;
...
void ChorusPresetBrowserComponent::confirmResetDialog()
{
    if (dialogMode != DialogMode::confirmReset)
        return;
...
void ChorusPresetBrowserComponent::deletePresetAtRow(int row)
{
...
void ChorusPresetBrowserComponent::launchImportChooser()
{
    fileChooser = std::make_unique<juce::FileChooser>("Import SSP Chorus Preset", juce::File{}, "*.sspchoruspreset");
    auto* chooser = fileChooser.get();
    juce::Component::SafePointer<ChorusPresetBrowserComponent> safeThis(this);
...
void ChorusPresetBrowserComponent::launchExportChooser()
{
    const auto baseName = processor.getCurrentPresetName().isNotEmpty() ? processor.getCurrentPresetName() : "Current Settings";
    fileChooser = std::make_unique<juce::FileChooser>("Export SSP Chorus Preset",
                                                      juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile(baseName + ".sspchoruspreset"),
                                                      "*.sspchoruspreset");
    auto* chooser = fileChooser.get();
    juce::Component::SafePointer<ChorusPresetBrowserComponent> safeThis(this);
...
juce::Colour ChorusPresetBrowserComponent::accentColour() const
{
    return juce::Colour(0xff48d8cb);
}

juce::String ChorusPresetBrowserComponent::categorySegment(const juce::String& category, int depth) const
{
...
bool ChorusPresetBrowserComponent::presetMatchesFilter(const PluginProcessor::PresetRecord& preset) const
{
...
bool ChorusPresetBrowserComponent::presetMatchesSearch(const PluginProcessor::PresetRecord& preset) const
{
