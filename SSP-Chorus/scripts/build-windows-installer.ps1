...
$artefactsDir = Join-Path $buildDir "SSPChorus_artefacts\$Config"
...
function Get-ProjectVersion { $cmake = Get-Content (Join-Path $projectRoot "CMakeLists.txt"); $match = $cmake | Select-String 'project\(SSP_CHORUS VERSION ([0-9]+\.[0-9]+\.[0-9]+)'; if ($match) { return $match.Matches[0].Groups[1].Value }; return "0.1.0" }
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Chorus.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Chorus.exe"
$zipPath = Join-Path $distDir "SSP-Chorus-Windows-Unsigned-Test-Package-$version.zip"
Build-TargetOrUseExisting -Target "SSPChorus_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPChorus_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) { $env:SSP_PROJECT_ROOT = $projectRoot; Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Chorus.iss"); Write-Host "Installer created in $distDir"; exit 0 }
