...
$artefactsDir = Join-Path $buildDir "SSPComb_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_COMB VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Comb.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Comb.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Comb.exe"
$zipPath = Join-Path $distDir "SSP-Comb-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Comb-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPComb_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPComb_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Comb.iss")
