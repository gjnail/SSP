...
#define BuildArtefactsDir AddBackslash(ProjectRoot) + "build\\SSPComb_artefacts\\Release"
#define StandaloneSource AddBackslash(BuildArtefactsDir) + "Standalone\\SSP Comb.exe"
#define VST3Source AddBackslash(BuildArtefactsDir) + "VST3\\SSP Comb.vst3"
#define ReadmeSource AddBackslash(ProjectRoot) + "packaging\\windows\\installer-readme.txt"

[Setup]
AppId={{2BF41215-EB32-4E37-99A7-E9D4EB3FF83C}
AppName={#MyAppName}
AppVerName={#MyAppName} {#MyAppVersion}
AppVersion={#MyAppVersion}
AppPublisher={#MyAppPublisher}
DefaultDirName={autopf64}\Stupid Simple Plugins\SSP Comb
DefaultGroupName=Stupid Simple Plugins
UninstallDisplayIcon={app}\SSP Comb.exe
...
OutputBaseFilename=SSP-Comb-Windows-Installer-{#MyAppVersion}
...
[Icons]
Name: "{group}\SSP Comb"; Filename: "{app}\SSP Comb.exe"; Components: standalone
Name: "{autodesktop}\SSP Comb"; Filename: "{app}\SSP Comb.exe"; Tasks: desktopicon; Components: standalone
Name: "{group}\Uninstall SSP Comb"; Filename: "{uninstallexe}"

[Run]
Filename: "{app}\SSP Comb.exe"; Description: "Launch SSP Comb"; Flags: nowait postinstall skipifsilent; Components: standalone

[Messages]
WizardSelectComponents=Choose which parts of SSP Comb to install.
...
standaloneComponentDescription=Installs the desktop version of SSP Comb.
desktopiconTaskDescription=Adds a desktop shortcut for the standalone app.
VST3DirPageTitle=Choose VST3 Install Location
VST3DirPageSubTitle=Select where SSP Comb.vst3 should be installed.
...
begin
  Result := ExpandConstant('{commoncf64}\VST3\SSP Comb.vst3');
end;
