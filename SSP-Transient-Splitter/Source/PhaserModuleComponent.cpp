...
struct PhaserSyncDivision
{
    const char* name;
    float beats = 1.0f;
};

const std::array<PhaserSyncDivision, 13>& getPhaserSyncDivisions()
{
    static const std::array<PhaserSyncDivision, 13> divisions{{
        { "1/32", 0.125f },
        { "1/16T", 1.0f / 6.0f },
        { "1/16", 0.25f },
        { "1/8T", 1.0f / 3.0f },
...
{
    const auto maxIndex = (int) getPhaserSyncDivisions().size() - 1;
...
float normalToCentreFrequency(float value)
{
...
float centreFrequencyToNormal(float hz)
{
    constexpr float minFrequency = 120.0f;
    constexpr float maxFrequency = 4200.0f;
    return juce::jlimit(0.0f, 1.0f, (float) juce::mapFromLog10(juce::jlimit(minFrequency, maxFrequency, hz), minFrequency, maxFrequency));
}
...
PhaserModuleComponent::PhaserModuleComponent(PluginProcessor& processor, juce::AudioProcessorValueTreeState& state, int slot)
    : apvts(state),
      slotIndex(slot)
{
    const auto accent = juce::Colour(0xff4d8dff);
    ...
    subtitleLabel.setText("Classic four-stage phase sweep with controllable depth, feedback, and centre focus for liquid movement or sharper notches.", juce::dontSendNotification);
    ...
    const std::array<juce::String, 5> controlNames{ "Rate", "Depth", "Feedback", "Center", "Mix" };
    ...
    controls[1].valueFromTextFunction = [] (const juce::String& text) -> double
    {
        return std::clamp(text.upToFirstOccurrenceOf("%", false, false).getDoubleValue() / 100.0, 0.0, 1.0);
    };

    controls[2].textFromValueFunction = [] (double value)
    {
        return juce::String(juce::roundToInt(juce::jmap((float) value, -85.0f, 85.0f))) + "%";
    };
    controls[2].valueFromTextFunction = [] (const juce::String& text) -> double
    {
        const auto mapped = (double) juce::jmap((float) text.upToFirstOccurrenceOf("%", false, false).getDoubleValue(),
                                                -85.0f, 85.0f, 0.0f, 1.0f);
        return std::clamp(mapped, 0.0, 1.0);
    };

    controls[3].textFromValueFunction = [] (double value)
    {
        const float frequency = normalToCentreFrequency((float) value);
    ...
    controls[3].valueFromTextFunction = [] (const juce::String& text)
    {
        ...
        const double raw = text.retainCharacters("0123456789.").getDoubleValue();
        return (double) centreFrequencyToNormal((float) (isKhz ? raw * 1000.0 : raw));
    };
    ...
    controls[4].valueFromTextFunction = [] (const juce::String& text) -> double
    {
        return std::clamp(text.upToFirstOccurrenceOf("%", false, false).getDoubleValue() / 100.0, 0.0, 1.0);
    };
...
int PhaserModuleComponent::getPreferredHeight() const
{
    return 332;
}

void PhaserModuleComponent::paint(juce::Graphics& g)
{
    const auto accent = juce::Colour(0xff4d8dff);
...
void PhaserModuleComponent::resized()
{
...
void PhaserModuleComponent::timerCallback()
{
    syncSyncButton();
    updateRateFormatting();
...
void PhaserModuleComponent::setSyncEnabled(bool shouldSync)
{
...
bool PhaserModuleComponent::isSyncEnabled() const
{
...
void PhaserModuleComponent::syncSyncButton()
{
...
void PhaserModuleComponent::updateRateFormatting()
{
    if (isSyncEnabled())
    {
        controls[0].textFromValueFunction = [] (double value)
        {
            return juce::String(getPhaserSyncDivisions()[(size_t) normalToSyncDivisionIndex((float) value)].name);
        };
        controls[0].valueFromTextFunction = [] (const juce::String& text)
        {
            const auto divisions = getPhaserSyncDivisions();
