...
juce::String getTypeDescription(int typeIndex)
{
    switch (typeIndex)
    {
        case 1: return "Classic crunchy lane with obvious grit but still enough body to stay musical.";
        case 2: return "Retro console-style reduction with chunkier steps and a drier, older texture.";
        case 3: return "Sharper modern reduction with brighter edges and more transient chewing.";
        case 4: return "Extreme smashed setting for zippery, broken, aggressively aliased destruction.";
        default: return "Cleaner digital reduction with softer aliasing and a more mix-friendly texture.";
    }
}
}

BitcrusherModuleComponent::BitcrusherModuleComponent(PluginProcessor& processor, juce::AudioProcessorValueTreeState& state, int slot)
    : apvts(state),
      slotIndex(slot)
{
    const auto accent = juce::Colour(0xffb38cff);
    ...
    const std::array<juce::String, 5> controlNames{ "Bits", "Rate", "Jitter", "Tone", "Mix" };
    ...
    {
        return juce::String(normalToBitDepth((float) value)) + " bit";
    };
    controls[1].textFromValueFunction = [] (double value)
    {
        return juce::String(normalToHoldSamples((float) value)) + " smp";
    };
    ...
    {
        const float hz = normalToToneHz((float) value);
    ...
    typeSelector.addItemList(PluginProcessor::getFXBitcrusherTypeNames(), 1);
...
int BitcrusherModuleComponent::getPreferredHeight() const
{
    return 386;
}

int BitcrusherModuleComponent::getSelectedType() const
{
    if (auto* raw = apvts.getRawParameterValue(getFXSlotVariantParamID(slotIndex)))
        return juce::jlimit(0, PluginProcessor::getFXBitcrusherTypeNames().size() - 1, (int) std::round(raw->load()));

    return 0;
}

void BitcrusherModuleComponent::setSelectedType(int typeIndex)
{
    setChoiceParameterValue(apvts,
                            getFXSlotVariantParamID(slotIndex),
                            juce::jlimit(0, PluginProcessor::getFXBitcrusherTypeNames().size() - 1, typeIndex));
    syncTypeSelector();
    updateDescription();
    repaint();
}

void BitcrusherModuleComponent::syncTypeSelector()
{
...
void BitcrusherModuleComponent::updateDescription()
{
    subtitleLabel.setText(getTypeDescription(getSelectedType()), juce::dontSendNotification);
}

void BitcrusherModuleComponent::paint(juce::Graphics& g)
{
    const auto accent = juce::Colour(0xffb38cff);
...
void BitcrusherModuleComponent::resized()
{
...
void BitcrusherModuleComponent::timerCallback()
{
    syncTypeSelector();
    updateDescription();
