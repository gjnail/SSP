...
$artefactsDir = Join-Path $buildDir "SSPLoud_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_LOUD VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Loud.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Loud.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Loud.exe"
$zipPath = Join-Path $distDir "SSP-Loud-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Loud-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPLoud_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPLoud_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Loud.iss")
