...
#define BuildArtefactsDir AddBackslash(ProjectRoot) + "build\\SSPDelay_artefacts\\Release"
#define StandaloneSource AddBackslash(BuildArtefactsDir) + "Standalone\\SSP Delay.exe"
#define VST3Source AddBackslash(BuildArtefactsDir) + "VST3\\SSP Delay.vst3"
#define ReadmeSource AddBackslash(ProjectRoot) + "packaging\\windows\\installer-readme.txt"

[Setup]
AppId={{00CE2B5C-E081-4A28-95B8-06D8BA6B4D16}
AppName={#MyAppName}
AppVerName={#MyAppName} {#MyAppVersion}
AppVersion={#MyAppVersion}
AppPublisher={#MyAppPublisher}
DefaultDirName={autopf64}\Stupid Simple Plugins\SSP Delay
DefaultGroupName=Stupid Simple Plugins
UninstallDisplayIcon={app}\SSP Delay.exe
...
OutputBaseFilename=SSP-Delay-Windows-Installer-{#MyAppVersion}
...
[Icons]
Name: "{group}\SSP Delay"; Filename: "{app}\SSP Delay.exe"; Components: standalone
Name: "{autodesktop}\SSP Delay"; Filename: "{app}\SSP Delay.exe"; Tasks: desktopicon; Components: standalone
Name: "{group}\Uninstall SSP Delay"; Filename: "{uninstallexe}"

[Run]
Filename: "{app}\SSP Delay.exe"; Description: "Launch SSP Delay"; Flags: nowait postinstall skipifsilent; Components: standalone

[Messages]
WizardSelectComponents=Choose which parts of SSP Delay to install.
...
standaloneComponentDescription=Installs the desktop version of SSP Delay.
desktopiconTaskDescription=Adds a desktop shortcut for the standalone app.
VST3DirPageTitle=Choose VST3 Install Location
VST3DirPageSubTitle=Select where SSP Delay.vst3 should be installed.
...
begin
  Result := ExpandConstant('{commoncf64}\VST3\SSP Delay.vst3');
end;
