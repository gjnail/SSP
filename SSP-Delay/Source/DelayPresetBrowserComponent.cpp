...
DelayPresetBrowserComponent::DelayPresetBrowserComponent(PluginProcessor& p)
    : processor(p)
{
...
void DelayPresetBrowserComponent::setAnchorBounds(juce::Rectangle<int> button, juce::Rectangle<int> graph)
{
    anchorBounds = button;
    graphBounds = graph;
    updateMenuLayout();
...
void DelayPresetBrowserComponent::open()
{
...
void DelayPresetBrowserComponent::close()
{
    endPreviewIfNeeded();
    hideDialog();
...
bool DelayPresetBrowserComponent::handleKeyPress(const juce::KeyPress& key)
{
...
void DelayPresetBrowserComponent::paint(juce::Graphics& g)
{
    if (! visibleOrAnimating)
        return;
...
void DelayPresetBrowserComponent::resized()
{
    updateMenuLayout();
    updateDialogLayout();
...
void DelayPresetBrowserComponent::mouseMove(const juce::MouseEvent& event)
{
    if (dialogMode != DialogMode::none)
        return;
...
void DelayPresetBrowserComponent::mouseExit(const juce::MouseEvent&)
{
    clearHoverPreviewPending();
    endPreviewIfNeeded();
}

void DelayPresetBrowserComponent::mouseDown(const juce::MouseEvent& event)
{
    if (dialogMode != DialogMode::none)
    {
        auto dialogBounds = juce::Rectangle<float>(dialogWidth, dialogMode == DialogMode::deleteUserPreset ? 340.0f : dialogHeight)
...
void DelayPresetBrowserComponent::mouseWheelMove(const juce::MouseEvent& event, const juce::MouseWheelDetails& wheel)
{
...
int DelayPresetBrowserComponent::getNumRows()
{
    return deletableUserPresets.size();
}

void DelayPresetBrowserComponent::paintListBoxItem(int rowNumber, juce::Graphics& g, int width, int height, bool rowIsSelected)
{
...
void DelayPresetBrowserComponent::listBoxItemClicked(int row, const juce::MouseEvent& event)
{
    if (event.x >= deleteListBox.getWidth() - 84)
        deletePresetAtRow(row);
}

void DelayPresetBrowserComponent::timerCallback()
{
    if (! visibleOrAnimating)
    {
        stopTimer();
...
void DelayPresetBrowserComponent::rebuildMenu()
{
    panels.clear();
    MenuPanelState root;
...
std::vector<DelayPresetBrowserComponent::MenuItem> DelayPresetBrowserComponent::buildRootMenuItems() const
{
...
std::vector<DelayPresetBrowserComponent::MenuItem> DelayPresetBrowserComponent::buildCategoryItems(const juce::Array<PluginProcessor::PresetRecord>& presets, int depth) const
{
    std::vector<MenuItem> items;
...
void DelayPresetBrowserComponent::updateMenuLayout()
{
    if (panels.empty())
        return;
...
void DelayPresetBrowserComponent::updateDialogLayout()
{
...
void DelayPresetBrowserComponent::setHighlightedItem(int panelIndex, int itemIndex, bool openChildren)
{
...
void DelayPresetBrowserComponent::moveHighlight(int delta)
{
    if (panels.empty())
        return;
...
void DelayPresetBrowserComponent::openHighlightedSubmenu()
{
    if (panels.empty())
        return;
...
void DelayPresetBrowserComponent::closeLastSubmenu()
{
    if (panels.size() > 1)
    {
        panels.pop_back();
...
void DelayPresetBrowserComponent::activateHighlightedItem()
{
    if (panels.empty())
        return;
...
int DelayPresetBrowserComponent::findNextSelectableIndex(const MenuPanelState& panel, int startIndex, int direction) const
{
...
float DelayPresetBrowserComponent::getItemHeight(const MenuItem& item) const
{
    return item.kind == MenuItem::Kind::divider ? dividerHeight : rowHeight;
}

float DelayPresetBrowserComponent::getPanelContentHeight(const MenuPanelState& panel) const
{
    float total = 0.0f;
...
juce::Rectangle<float> DelayPresetBrowserComponent::getPanelContentBounds(const MenuPanelState& panel) const
{
    return panel.bounds.reduced(10.0f, 10.0f);
}

juce::Rectangle<float> DelayPresetBrowserComponent::getItemBounds(const MenuPanelState& panel, int itemIndex) const
{
...
bool DelayPresetBrowserComponent::isPointInsideAnyPanel(juce::Point<float> point) const
{
    for (const auto& panel : panels)
...
std::pair<int, int> DelayPresetBrowserComponent::hitTestItem(juce::Point<float> point) const
{
...
void DelayPresetBrowserComponent::updateHoverState(juce::Point<float> point)
{
...
void DelayPresetBrowserComponent::clearHoverPreviewPending()
{
    pendingPreviewPresetKey.clear();
    pendingPreviewDeadline = 0;
}

void DelayPresetBrowserComponent::maybeBeginHoverPreview()
{
...
void DelayPresetBrowserComponent::endPreviewIfNeeded()
{
    processor.endPresetPreview();
}

void DelayPresetBrowserComponent::executeMenuItem(const MenuItem& item)
{
    if (! item.enabled)
        return;
...
void DelayPresetBrowserComponent::executeAction(const juce::String& actionID)
{
    if (actionID == "save-as")
    {
        showSaveDialog();
...
void DelayPresetBrowserComponent::loadPreset(const PluginProcessor::PresetRecord& preset)
{
    endPreviewIfNeeded();
...
void DelayPresetBrowserComponent::showSaveDialog()
{
...
void DelayPresetBrowserComponent::showDeleteDialog()
{
...
void DelayPresetBrowserComponent::showResetDialog()
{
    dialogMode = DialogMode::confirmReset;
    dialogTitleLabel.setText("Reset User Presets", juce::dontSendNotification);
    dialogMessageLabel.setText("This removes all user-saved SSP Delay presets from your local library. Factory presets stay intact.", juce::dontSendNotification);
...
void DelayPresetBrowserComponent::hideDialog()
{
...
void DelayPresetBrowserComponent::confirmSaveDialog()
{
    if (dialogMode != DialogMode::saveAs)
        return;
...
void DelayPresetBrowserComponent::confirmResetDialog()
{
    if (dialogMode != DialogMode::confirmReset)
        return;
...
void DelayPresetBrowserComponent::deletePresetAtRow(int row)
{
...
void DelayPresetBrowserComponent::launchImportChooser()
{
    fileChooser = std::make_unique<juce::FileChooser>("Import SSP Delay Preset", juce::File{}, "*.sspdelaypreset");
    auto* chooser = fileChooser.get();
    juce::Component::SafePointer<DelayPresetBrowserComponent> safeThis(this);
...
void DelayPresetBrowserComponent::launchExportChooser()
{
    const auto baseName = processor.getCurrentPresetName().isNotEmpty() ? processor.getCurrentPresetName() : "Current Settings";
    fileChooser = std::make_unique<juce::FileChooser>("Export SSP Delay Preset",
                                                      juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile(baseName + ".sspdelaypreset"),
                                                      "*.sspdelaypreset");
    auto* chooser = fileChooser.get();
    juce::Component::SafePointer<DelayPresetBrowserComponent> safeThis(this);
...
juce::Colour DelayPresetBrowserComponent::accentColour() const
{
    return juce::Colour(0xff48d8cb);
}

juce::String DelayPresetBrowserComponent::categorySegment(const juce::String& category, int depth) const
{
...
bool DelayPresetBrowserComponent::presetMatchesFilter(const PluginProcessor::PresetRecord& preset) const
{
...
bool DelayPresetBrowserComponent::presetMatchesSearch(const PluginProcessor::PresetRecord& preset) const
{
