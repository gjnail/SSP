...
$artefactsDir = Join-Path $buildDir "SSPDelay_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_DELAY VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Delay.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Delay.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Delay.exe"
$zipPath = Join-Path $distDir "SSP-Delay-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Delay-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPDelay_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPDelay_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Delay.iss")
