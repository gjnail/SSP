...
namespace
{
midibuddyui::LookAndFeel lookAndFeel;
...
juce::Colour brandTeal() { return juce::Colour(0xff3ecfff); }
juce::Colour brandAmber() { return juce::Colour(0xffffcf4d); }
juce::Colour brandIce() { return juce::Colour(0xff9de7ff); }
...
void drawEditorBackdrop(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    g.setColour(brandAmber().withAlpha(0.08f));
    g.fillEllipse(bounds.getX() - bounds.getWidth() * 0.16f,
                  bounds.getY() - bounds.getWidth() * 0.16f,
                  bounds.getWidth() * 0.40f,
                  bounds.getWidth() * 0.40f);

    g.setColour(brandTeal().withAlpha(0.06f));
    g.fillEllipse(bounds.getRight() - bounds.getWidth() * 0.22f,
                  bounds.getBottom() - bounds.getWidth() * 0.22f,
...
void LookAndFeel::drawButtonBackground(juce::Graphics& g, juce::Button& button, const juce::Colour&, bool over, bool down)
{
    ...
    auto lensTop = button.findColour(juce::TextButton::buttonColourId);
    auto lensBottom = lensTop.darker(0.28f);
    ...
    if (down)
    {
        lensTop = lensTop.darker(0.08f);
        lensBottom = lensBottom.darker(0.10f);
    }
    ...
    g.setColour(juce::Colour(0xfffff4d1).withAlpha(0.92f));
