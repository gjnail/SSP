...
$artefactsDir = Join-Path $buildDir "SSPPrism_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_PRISM VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Prism.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Prism.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Prism.exe"
$zipPath = Join-Path $distDir "SSP-Prism-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Prism-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPPrism_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPPrism_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Prism.iss")
