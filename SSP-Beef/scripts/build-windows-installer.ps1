...
$artefactsDir = Join-Path $buildDir "SSPBeef_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_BEEF VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Beef.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Beef.exe"
$zipPath = Join-Path $distDir "SSP-Beef-Windows-Unsigned-Test-Package-$version.zip"

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPBeef_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPBeef_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Beef.iss")
