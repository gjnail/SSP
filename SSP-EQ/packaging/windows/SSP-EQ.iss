...
#define BuildArtefactsDir AddBackslash(ProjectRoot) + "build\\SSPEQ_artefacts\\Release"
#define StandaloneSource AddBackslash(BuildArtefactsDir) + "Standalone\\SSP EQ.exe"
#define VST3Source AddBackslash(BuildArtefactsDir) + "VST3\\SSP EQ.vst3"
#define ReadmeSource AddBackslash(ProjectRoot) + "packaging\\windows\\installer-readme.txt"

[Setup]
AppId={{B90ED5D3-C3A0-4D95-8A25-F2D57A2B3799}
AppName={#MyAppName}
AppVerName={#MyAppName} {#MyAppVersion}
AppVersion={#MyAppVersion}
AppPublisher={#MyAppPublisher}
DefaultDirName={autopf64}\Stupid Simple Plugins\SSP EQ
DefaultGroupName=Stupid Simple Plugins
UninstallDisplayIcon={app}\SSP EQ.exe
...
OutputBaseFilename=SSP-EQ-Windows-Installer-{#MyAppVersion}
...
[Icons]
Name: "{group}\SSP EQ"; Filename: "{app}\SSP EQ.exe"; Components: standalone
Name: "{autodesktop}\SSP EQ"; Filename: "{app}\SSP EQ.exe"; Tasks: desktopicon; Components: standalone
Name: "{group}\Uninstall SSP EQ"; Filename: "{uninstallexe}"

[Run]
Filename: "{app}\SSP EQ.exe"; Description: "Launch SSP EQ"; Flags: nowait postinstall skipifsilent; Components: standalone
...
begin
  Result := ExpandConstant('{commoncf64}\VST3\SSP EQ.vst3');
end;

procedure InitializeWizard;
begin
  VST3DirPage := CreateInputDirPage(wpSelectDir, 'Choose VST3 Install Location', 'Select where SSP EQ.vst3 should be installed.', 'The standard VST3 path is recommended, but you can choose a custom folder if your setup needs it.', False, '');
  VST3DirPage.Add('');
