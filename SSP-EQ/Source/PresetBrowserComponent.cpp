...
void PresetBrowserComponent::setAnchorBounds(juce::Rectangle<int> button, juce::Rectangle<int> graph)
{
    anchorBounds = button;
    graphBounds = graph;
    updateMenuLayout();
...
void PresetBrowserComponent::mouseMove(const juce::MouseEvent& event)
{
    if (dialogMode != DialogMode::none)
        return;
...
void PresetBrowserComponent::mouseWheelMove(const juce::MouseEvent& event, const juce::MouseWheelDetails& wheel)
{
...
void PresetBrowserComponent::timerCallback()
{
    ...
    openProgress += (target - openProgress) * 0.24f;
    ...
    else if (! visibleOrAnimating)
    {
        stopTimer();
    }
}

void PresetBrowserComponent::rebuildMenu()
{
    panels.clear();
    MenuPanelState root;
...
std::vector<PresetBrowserComponent::MenuItem> PresetBrowserComponent::buildRootMenuItems() const
{
    ...
    else
    {
        ...
        {
            MenuItem surround;
            surround.kind = MenuItem::Kind::placeholder;
            surround.label = "Surround";
            surround.children.push_back(MenuItem{ MenuItem::Kind::placeholder, "Coming Soon", {}, false, false, false, {}, {} });
            items.push_back(std::move(surround));
        }
    ...
    options.children.push_back(MenuItem{ MenuItem::Kind::action, "A/B Comparison", "ab-compare", processor.isABComparisonActive(), processor.hasABComparisonTarget(), false, {}, {} });
    items.push_back(std::move(options));

    return items;
}

std::vector<PresetBrowserComponent::MenuItem> PresetBrowserComponent::buildCategoryItems(const juce::Array<PluginProcessor::PresetRecord>& presets, int depth) const
{
    std::vector<MenuItem> items;
...
void PresetBrowserComponent::updateMenuLayout()
{
    if (panels.empty())
        return;
...
void PresetBrowserComponent::updateDialogLayout()
{
...
void PresetBrowserComponent::setHighlightedItem(int panelIndex, int itemIndex, bool openChildren)
{
...
void PresetBrowserComponent::moveHighlight(int delta)
{
    if (panels.empty())
        return;
...
void PresetBrowserComponent::openHighlightedSubmenu()
{
    if (panels.empty())
        return;
...
void PresetBrowserComponent::closeLastSubmenu()
{
    if (panels.size() > 1)
    {
        panels.pop_back();
...
void PresetBrowserComponent::activateHighlightedItem()
{
    if (panels.empty())
        return;
...
int PresetBrowserComponent::findNextSelectableIndex(const MenuPanelState& panel, int startIndex, int direction) const
{
...
float PresetBrowserComponent::getItemHeight(const MenuItem& item) const
{
    return item.kind == MenuItem::Kind::divider ? dividerHeight : rowHeight;
}

float PresetBrowserComponent::getPanelContentHeight(const MenuPanelState& panel) const
{
    float total = 0.0f;
...
juce::Rectangle<float> PresetBrowserComponent::getPanelContentBounds(const MenuPanelState& panel) const
{
    return panel.bounds.reduced(10.0f, 10.0f);
}

juce::Rectangle<float> PresetBrowserComponent::getItemBounds(const MenuPanelState& panel, int itemIndex) const
{
...
bool PresetBrowserComponent::isPointInsideAnyPanel(juce::Point<float> point) const
{
    for (const auto& panel : panels)
...
std::pair<int, int> PresetBrowserComponent::hitTestItem(juce::Point<float> point) const
{
...
void PresetBrowserComponent::updateHoverState(juce::Point<float> point)
{
...
void PresetBrowserComponent::clearHoverPreviewPending()
{
    pendingPreviewPresetKey.clear();
    pendingPreviewDeadline = 0;
}

void PresetBrowserComponent::maybeBeginHoverPreview()
{
...
void PresetBrowserComponent::endPreviewIfNeeded()
{
    processor.endPresetPreview();
}

void PresetBrowserComponent::executeMenuItem(const MenuItem& item)
{
    if (! item.enabled)
        return;
...
void PresetBrowserComponent::executeAction(const juce::String& actionID)
{
    if (actionID == "save-as")
    {
        showSaveDialog();
...
void PresetBrowserComponent::loadPreset(const PluginProcessor::PresetRecord& preset)
{
    endPreviewIfNeeded();
...
void PresetBrowserComponent::showDeleteDialog()
{
...
void PresetBrowserComponent::showResetDialog()
{
    dialogMode = DialogMode::confirmReset;
    dialogTitleLabel.setText("Reset User Presets", juce::dontSendNotification);
    dialogMessageLabel.setText("This removes all user-saved SSP EQ presets from your local library. Factory presets stay intact.", juce::dontSendNotification);
...
void PresetBrowserComponent::hideDialog()
{
...
void PresetBrowserComponent::confirmSaveDialog()
{
    if (dialogMode != DialogMode::saveAs)
        return;
...
void PresetBrowserComponent::confirmResetDialog()
{
    if (dialogMode != DialogMode::confirmReset)
        return;
...
void PresetBrowserComponent::deletePresetAtRow(int row)
{
...
{
    fileChooser = std::make_unique<juce::FileChooser>("Import SSP EQ Preset", juce::File{}, "*.sspeqpreset");
...
void PresetBrowserComponent::launchExportChooser()
{
    ...
    fileChooser = std::make_unique<juce::FileChooser>("Export SSP EQ Preset",
                                                      juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile(baseName + ".sspeqpreset"),
                                                      "*.sspeqpreset");
    auto* chooser = fileChooser.get();
    juce::Component::SafePointer<PresetBrowserComponent> safeThis(this);
...
juce::Colour PresetBrowserComponent::accentColour() const
{
    return juce::Colour(0xff48d8cb);
}

juce::String PresetBrowserComponent::categorySegment(const juce::String& category, int depth) const
{
...
bool PresetBrowserComponent::presetMatchesFilter(const PluginProcessor::PresetRecord& preset) const
{
    if (filterMode == FilterMode::userOnly)
...
bool PresetBrowserComponent::presetMatchesSearch(const PluginProcessor::PresetRecord& preset) const
{
