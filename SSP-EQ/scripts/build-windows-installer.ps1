...
$artefactsDir = Join-Path $buildDir "SSPEQ_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_EQ VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP EQ.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP EQ.exe"
$zipPath = Join-Path $distDir "SSP-EQ-Windows-Unsigned-Test-Package-$version.zip"

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPEQ_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPEQ_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-EQ.iss")
