...
$buildDir = Join-Path $projectRoot "build-hyper"
$artefactsDir = Join-Path $buildDir "SSPHyperResonance_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_HYPER_RESONANCE VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Hyper Resonance.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Hyper Resonance.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Hyper Resonance.exe"
$zipPath = Join-Path $distDir "SSP-Hyper-Resonance-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Hyper-Resonance-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPHyperResonance_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPHyperResonance_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Hyper-Resonance.iss")
