...
$artefactsDir = Join-Path $buildDir "SSPSaturate_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_SATURATE VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Saturate.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Saturate.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Saturate.exe"
$zipPath = Join-Path $distDir "SSP-Saturate-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Saturate-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPSaturate_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPSaturate_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Saturate.iss")
