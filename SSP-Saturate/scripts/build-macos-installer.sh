...
ARTEFACTS_DIR="${BUILD_DIR}/SSPSaturate_artefacts/Release"
...
VERSION="$(sed -nE 's/^project\(SSP_SATURATE VERSION ([0-9]+\.[0-9]+\.[0-9]+).*/\1/p' "${PROJECT_ROOT}/CMakeLists.txt" | head -n1)"
VERSION="${VERSION:-0.1.0}"

cmake --build "${BUILD_DIR}" --config Release --target SSPSaturate_VST3
cmake --build "${BUILD_DIR}" --config Release --target SSPSaturate_Standalone

VST3_PATH="${ARTEFACTS_DIR}/VST3/SSP Saturate.vst3"
APP_PATH="${ARTEFACTS_DIR}/Standalone/SSP Saturate.app"
...
COMPONENT_PKG="${DIST_DIR}/SSP-Saturate-${VERSION}-component.pkg"
FINAL_PKG="${DIST_DIR}/SSP-Saturate-macOS-Installer-${VERSION}.pkg"

  pkgbuild \
    --root "${PKG_ROOT}" \
  --identifier "com.ssp.saturate.installer" \
    --version "${VERSION}" \
    --install-location "/" \
    "${COMPONENT_PKG}"
...
ZIP_PATH="${DIST_DIR}/SSP-Saturate-macOS-Package-${VERSION}.zip"
rm -f "${ZIP_PATH}"
