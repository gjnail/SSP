...
juce::Colour backgroundTop() { return juce::Colour(0xff09111a); }
juce::Colour backgroundBottom() { return juce::Colour(0xff111a23); }
juce::Colour panelTop() { return juce::Colour(0xff182431); }
juce::Colour panelBottom() { return juce::Colour(0xff101823); }
juce::Colour panelOutline() { return juce::Colour(0xff314654); }
juce::Colour textStrong() { return juce::Colour(0xffedf4fb); }
juce::Colour textMuted() { return juce::Colour(0xff8da2b4); }
} // namespace
...
{
    juce::ColourGradient bg(backgroundTop(), bounds.getX(), bounds.getY(),
                            backgroundBottom(), bounds.getRight(), bounds.getBottom(), false);
    bg.addColour(0.55, juce::Colour(0xff0c141d));
    g.setGradientFill(bg);
    g.fillRoundedRectangle(bounds, 18.0f);

    g.setColour(juce::Colour(0xff33cfff).withAlpha(0.08f));
    g.fillEllipse(bounds.getX() - 80.0f, bounds.getY() - 70.0f, bounds.getWidth() * 0.42f, bounds.getHeight() * 0.34f);
    g.setColour(juce::Colour(0xff23e0bc).withAlpha(0.06f));
    g.fillEllipse(bounds.getRight() - bounds.getWidth() * 0.32f, bounds.getBottom() - bounds.getHeight() * 0.26f,
                  bounds.getWidth() * 0.34f, bounds.getHeight() * 0.28f);

    g.setColour(juce::Colour(0xff04080c));
    g.drawRoundedRectangle(bounds.reduced(1.0f), 18.0f, 2.0f);
    g.setColour(juce::Colours::white.withAlpha(0.05f));
    g.drawRoundedRectangle(bounds.reduced(3.0f), 15.0f, 1.0f);
}

void drawPanelBackground(juce::Graphics& g, juce::Rectangle<float> bounds, juce::Colour accent, float radius)
{
    juce::DropShadow(juce::Colours::black.withAlpha(0.28f), 18, { 0, 7 }).drawForRectangle(g, bounds.getSmallestIntegerContainer());
    juce::ColourGradient fill(panelTop(), bounds.getX(), bounds.getY(), panelBottom(), bounds.getRight(), bounds.getBottom(), false);
    fill.addColour(0.42, juce::Colour(0xff13212d));
    g.setGradientFill(fill);
    g.fillRoundedRectangle(bounds, radius);

    auto headerBand = bounds.reduced(1.0f).removeFromTop(28.0f);
    juce::ColourGradient headerFill(accent.withAlpha(0.14f), headerBand.getX(), headerBand.getY(),
                                    juce::Colours::transparentBlack, headerBand.getRight(), headerBand.getBottom(), false);
    g.setGradientFill(headerFill);
    g.fillRoundedRectangle(headerBand, radius);

    g.setColour(accent.withAlpha(0.9f));
    g.fillRect(bounds.reduced(18.0f, 10.0f).removeFromTop(2.0f));
    g.setColour(juce::Colour(0xff05090d));
    g.drawRoundedRectangle(bounds.reduced(0.5f), radius, 1.5f);
    g.setColour(panelOutline());
    g.drawRoundedRectangle(bounds.reduced(1.8f), radius - 1.0f, 1.0f);
    g.setColour(juce::Colours::white.withAlpha(0.04f));
    g.drawRoundedRectangle(bounds.reduced(3.0f), radius - 1.6f, 1.0f);
}
...
void VectorLookAndFeel::drawToggleButton(juce::Graphics& g, juce::ToggleButton& button, bool shouldDrawButtonAsHighlighted, bool)
{
    ...
    g.setColour(shouldDrawButtonAsHighlighted ? textStrong() : textMuted().brighter(0.3f));
