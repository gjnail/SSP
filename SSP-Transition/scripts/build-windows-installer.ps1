...
$artefactsDir = Join-Path $buildDir "SSPTransition_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_TRANSITION VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Transition.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Transition.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Transition.exe"
$zipPath = Join-Path $distDir "SSP-Transition-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Transition-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPTransition_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPTransition_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Transition.iss")
