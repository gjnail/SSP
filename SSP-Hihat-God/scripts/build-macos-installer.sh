...
ARTEFACTS_DIR="${BUILD_DIR}/SSPHihatGod_artefacts/Release"
...
VERSION="$(sed -nE 's/^project\(SSP_HIHAT_GOD VERSION ([0-9]+\.[0-9]+\.[0-9]+).*/\1/p' "${PROJECT_ROOT}/CMakeLists.txt" | head -n1)"
VERSION="${VERSION:-0.1.0}"

cmake --build "${BUILD_DIR}" --config Release --target SSPHihatGod_VST3
cmake --build "${BUILD_DIR}" --config Release --target SSPHihatGod_Standalone

VST3_PATH="${ARTEFACTS_DIR}/VST3/SSP Hihat God.vst3"
APP_PATH="${ARTEFACTS_DIR}/Standalone/SSP Hihat God.app"
...
COMPONENT_PKG="${DIST_DIR}/SSP-Hihat-God-${VERSION}-component.pkg"
FINAL_PKG="${DIST_DIR}/SSP-Hihat-God-macOS-Installer-${VERSION}.pkg"

  pkgbuild \
    --root "${PKG_ROOT}" \
  --identifier "com.ssp.hihatgod.installer" \
    --version "${VERSION}" \
    --install-location "/" \
    "${COMPONENT_PKG}"
...
ZIP_PATH="${DIST_DIR}/SSP-Hihat-God-macOS-Package-${VERSION}.zip"
rm -f "${ZIP_PATH}"
