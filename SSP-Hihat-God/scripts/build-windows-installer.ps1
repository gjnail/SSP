...
$artefactsDir = Join-Path $buildDir "SSPHihatGod_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_HIHAT_GOD VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Hihat God.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Hihat God.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Hihat God.exe"
$zipPath = Join-Path $distDir "SSP-Hihat-God-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Hihat-God-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPHihatGod_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPHihatGod_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Hihat-God.iss")
