...
$artefactsDir = Join-Path $buildDir "SSPLimiter_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_LIMITER VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Limiter.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Limiter.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Limiter.exe"
$zipPath = Join-Path $distDir "SSP-Limiter-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Limiter-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPLimiter_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPLimiter_Standalone" -ExpectedPath $standalonePath
