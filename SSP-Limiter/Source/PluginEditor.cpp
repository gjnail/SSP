      ...
      presetBrowser(p)
{
    ...
    titleLabel.setText("SSP Limiter", juce::dontSendNotification);
    ...
    hintLabel.setText("Lookahead brickwall control with a large limiter scope, preset browser, and extended finish controls.",
...
void PluginEditor::resized()
{
    ...
    presetButton.setBounds(titleRow.removeFromLeft(260));
    titleRow.removeFromLeft(6);
    ...
    const int browserWidth = juce::jmin(620, getWidth() - 48);
...
bool PluginEditor::keyPressed(const juce::KeyPress& key)
{
    ...
    {
        if (auto* limiterProcessor = dynamic_cast<PluginProcessor*>(getAudioProcessor()))
            limiterProcessor->stepPreset(-1);
        return true;
    }
...
void PluginEditor::timerCallback()
{
    if (auto* limiterProcessor = dynamic_cast<PluginProcessor*>(getAudioProcessor()))
    {
        auto name = limiterProcessor->getCurrentPresetName();
        if (name.isEmpty())
            name = "Default Limiter";
        if (limiterProcessor->isCurrentPresetDirty())
            name << " *";
