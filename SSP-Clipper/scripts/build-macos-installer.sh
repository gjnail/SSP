...
ARTEFACTS_DIR="${BUILD_DIR}/SSPClipper_artefacts/Release"
...
VERSION="$(sed -nE 's/^project\(SSP_CLIPPER VERSION ([0-9]+\.[0-9]+\.[0-9]+).*/\1/p' "${PROJECT_ROOT}/CMakeLists.txt" | head -n1)"
VERSION="${VERSION:-0.1.0}"

cmake --build "${BUILD_DIR}" --config Release --target SSPClipper_VST3
cmake --build "${BUILD_DIR}" --config Release --target SSPClipper_Standalone

VST3_PATH="${ARTEFACTS_DIR}/VST3/SSP Clipper.vst3"
APP_PATH="${ARTEFACTS_DIR}/Standalone/SSP Clipper.app"
...
if command -v pkgbuild >/dev/null 2>&1 && command -v productbuild >/dev/null 2>&1; then
  ...
  COMPONENT_PKG="${DIST_DIR}/SSP-Clipper-${VERSION}-component.pkg"
  FINAL_PKG="${DIST_DIR}/SSP-Clipper-macOS-Installer-${VERSION}.pkg"

  pkgbuild \
    --root "${PKG_ROOT}" \
    --identifier "com.ssp.clipper.installer" \
    --version "${VERSION}" \
    --install-location "/" \
    "${COMPONENT_PKG}"
...
ZIP_PATH="${DIST_DIR}/SSP-Clipper-macOS-Package-${VERSION}.zip"
rm -f "${ZIP_PATH}"
