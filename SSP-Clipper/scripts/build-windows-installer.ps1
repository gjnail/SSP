...
$artefactsDir = Join-Path $buildDir "SSPClipper_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_CLIPPER VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Clipper.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Clipper.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Clipper.exe"
$zipPath = Join-Path $distDir "SSP-Clipper-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Clipper-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPClipper_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPClipper_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Clipper.iss")
