...
public:
    ...
    float getDepthMeterLevel() const noexcept;

    juce::AudioProcessorValueTreeState apvts;

private:
    static juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout();
    void updateMeters(float inputPeak, float outputPeak, float depthAmount) noexcept;

    std::array<float, 2> dcInputState{{0.0f, 0.0f}};
    std::array<float, 2> dcOutputState{{0.0f, 0.0f}};

    juce::SmoothedValue<float> driveSmoothed;
    juce::SmoothedValue<float> frameSmoothed;
    juce::SmoothedValue<float> phaseSmoothed;
    juce::SmoothedValue<float> biasSmoothed;
    juce::SmoothedValue<float> smoothSmoothed;
    juce::SmoothedValue<float> outputSmoothed;
    juce::SmoothedValue<float> mixSmoothed;

    std::atomic<float> inputMeter{0.0f};
    std::atomic<float> outputMeter{0.0f};
    std::atomic<float> depthMeter{0.0f};
