...
$artefactsDir = Join-Path $buildDir "SSP3DWaveshaper_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_3D_WAVESHAPER VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP 3D Waveshaper.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP 3D Waveshaper.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP 3D Waveshaper.exe"
$zipPath = Join-Path $distDir "SSP-3D-Waveshaper-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-3D-Waveshaper-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSP3DWaveshaper_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSP3DWaveshaper_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-3D-Waveshaper.iss")
