#define MyAppName "SSP 3D Waveshaper"
...
#define BuildArtefactsDir AddBackslash(ProjectRoot) + "build\\SSP3DWaveshaper_artefacts\\Release"
#define StandaloneSource AddBackslash(BuildArtefactsDir) + "Standalone\\SSP 3D Waveshaper.exe"
#define VST3Source AddBackslash(BuildArtefactsDir) + "VST3\\SSP 3D Waveshaper.vst3"
#define ReadmeSource AddBackslash(ProjectRoot) + "packaging\\windows\\installer-readme.txt"

[Setup]
AppId={{9F882A55-63BC-4A62-8EAD-D0F1CC6F94C8}
AppName={#MyAppName}
AppVerName={#MyAppName} {#MyAppVersion}
AppVersion={#MyAppVersion}
AppPublisher={#MyAppPublisher}
DefaultDirName={autopf64}\Stupid Simple Plugins\SSP 3D Waveshaper
DefaultGroupName=Stupid Simple Plugins
UninstallDisplayIcon={app}\SSP 3D Waveshaper.exe
...
OutputBaseFilename=SSP-3D-Waveshaper-Windows-Installer-{#MyAppVersion}
...
[Icons]
Name: "{group}\SSP 3D Waveshaper"; Filename: "{app}\SSP 3D Waveshaper.exe"; Components: standalone
Name: "{autodesktop}\SSP 3D Waveshaper"; Filename: "{app}\SSP 3D Waveshaper.exe"; Tasks: desktopicon; Components: standalone
Name: "{group}\Uninstall SSP 3D Waveshaper"; Filename: "{uninstallexe}"

[Run]
Filename: "{app}\SSP 3D Waveshaper.exe"; Description: "Launch SSP 3D Waveshaper"; Flags: nowait postinstall skipifsilent; Components: standalone

[Messages]
WizardSelectComponents=Choose which parts of SSP 3D Waveshaper to install.
...
standaloneComponentDescription=Installs the desktop version of SSP 3D Waveshaper.
desktopiconTaskDescription=Adds a desktop shortcut for the standalone app.
VST3DirPageTitle=Choose VST3 Install Location
VST3DirPageSubTitle=Select where SSP 3D Waveshaper.vst3 should be installed.
...
begin
  Result := ExpandConstant('{commoncf64}\VST3\SSP 3D Waveshaper.vst3');
end;
