#define MyAppName "SSP Vintage Compress"
...
#endif
#define BuildArtefactsDir AddBackslash(ProjectRoot) + "build\\SSPVintageCompress_artefacts\\Release"
#define StandaloneSource AddBackslash(BuildArtefactsDir) + "Standalone\\SSP Vintage Compress.exe"
#define VST3Source AddBackslash(BuildArtefactsDir) + "VST3\\SSP Vintage Compress.vst3"
#define ReadmeSource AddBackslash(ProjectRoot) + "packaging\\windows\\installer-readme.txt"
[Setup]
AppId={{33D45A31-CBA1-4456-9EAE-C4771C829E2A}
AppName={#MyAppName}
AppVerName={#MyAppName} {#MyAppVersion}
AppVersion={#MyAppVersion}
AppPublisher={#MyAppPublisher}
DefaultDirName={autopf64}\Stupid Simple Plugins\SSP Vintage Compress
DefaultGroupName=Stupid Simple Plugins
UninstallDisplayIcon={app}\SSP Vintage Compress.exe
...
OutputBaseFilename=SSP-Vintage-Compress-Windows-Installer-{#MyAppVersion}
...
[Run]
Filename: "{app}\SSP Vintage Compress.exe"; Description: "Launch SSP Vintage Compress"; Flags: nowait postinstall skipifsilent; Components: standalone
[Code]
var VST3DirPage: TInputDirWizardPage;
function DefaultVST3Dir: string;
begin
  Result := ExpandConstant('{commoncf64}\VST3\SSP Vintage Compress.vst3');
end;
procedure InitializeWizard;
begin
  VST3DirPage := CreateInputDirPage(wpSelectDir, 'Choose VST3 Install Location', 'Select where SSP Vintage Compress.vst3 should be installed.', 'The standard VST3 path is recommended, but you can choose a custom folder if your setup needs it.', False, '');
  VST3DirPage.Add('');
