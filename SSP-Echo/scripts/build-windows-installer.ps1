...
$artefactsDir = Join-Path $buildDir "SSPEcho_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_ECHO VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Echo.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Echo.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Echo.exe"
$zipPath = Join-Path $distDir "SSP-Echo-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Echo-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPEcho_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPEcho_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Echo.iss")
