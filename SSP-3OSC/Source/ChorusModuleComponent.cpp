...
const std::array<ChorusSyncDivision, 13>& getChorusSyncDivisions()
{
...
{
    const auto maxIndex = (int) getChorusSyncDivisions().size() - 1;
...
{
    return juce::jlimit(0.0f, 1.0f, juce::jmap(ms, 3.0f, 28.0f, 0.0f, 1.0f));
}
...
{
    return juce::jlimit(0.0f, 1.0f, juce::jmap(feedback, -0.7f, 0.7f, 0.0f, 1.0f));
}
...
ChorusModuleComponent::ChorusModuleComponent(PluginProcessor& processor, juce::AudioProcessorValueTreeState& state, int slot)
    : apvts(state),
      slotIndex(slot)
{
    const auto accent = juce::Colour(0xff6ef0d6);
    ...
    subtitleLabel.setText("Stereo thickener with rate, depth, delay, and feedback controls for everything from widening to liquid swirl.", juce::dontSendNotification);
    ...
    {
        return juce::String(normalToDelayMs((float) value), 1) + " ms";
    };
...
int ChorusModuleComponent::getPreferredHeight() const
{
    return 332;
}

void ChorusModuleComponent::paint(juce::Graphics& g)
{
    const auto accent = juce::Colour(0xff6ef0d6);
...
void ChorusModuleComponent::resized()
{
...
void ChorusModuleComponent::timerCallback()
{
    syncSyncButton();
    updateRateFormatting();
...
void ChorusModuleComponent::setSyncEnabled(bool shouldSync)
{
...
bool ChorusModuleComponent::isSyncEnabled() const
{
...
void ChorusModuleComponent::syncSyncButton()
{
...
void ChorusModuleComponent::updateRateFormatting()
{
    if (isSyncEnabled())
    {
        controls[0].textFromValueFunction = [] (double value)
        {
            return juce::String(getChorusSyncDivisions()[(size_t) normalToSyncDivisionIndex((float) value)].name);
        };
        controls[0].valueFromTextFunction = [] (const juce::String& text)
        {
            const auto divisions = getChorusSyncDivisions();
