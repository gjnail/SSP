...
$artefactsDir = Join-Path $buildDir "SSPSubValidator_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_SUB_VALIDATOR VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Sub Validator.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Sub Validator.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Sub Validator.exe"
$zipPath = Join-Path $distDir "SSP-Sub-Validator-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Sub-Validator-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPSubValidator_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPSubValidator_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Sub-Validator.iss")
