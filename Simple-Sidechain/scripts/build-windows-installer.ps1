...
$artefactsDir = Join-Path $buildDir "SSPSimpleSidechain_artefacts\$Config"
...
function Get-ProjectVersion {
    ...
    $match = $cmake | Select-String 'project\(SSP_SIMPLE_SIDECHAIN VERSION ([0-9]+\.[0-9]+\.[0-9]+)'
    if ($match) {
...
$vst3Path = Join-Path $artefactsDir "VST3\SSP Simple Sidechain.vst3"
$vst3BinaryPath = Join-Path $vst3Path "Contents\x86_64-win\SSP Simple Sidechain.vst3"
$standalonePath = Join-Path $artefactsDir "Standalone\SSP Simple Sidechain.exe"
$zipPath = Join-Path $distDir "SSP-Simple-Sidechain-Windows-Unsigned-Test-Package-$version.zip"
$installerPath = Join-Path $buildDir "dist\windows\SSP-Simple-Sidechain-Windows-Installer-$version.exe"
$signingConfig = Get-SigningConfig

Write-Host "Building Release artefacts..."
Build-TargetOrUseExisting -Target "SSPSimpleSidechain_VST3" -ExpectedPath $vst3Path
Build-TargetOrUseExisting -Target "SSPSimpleSidechain_Standalone" -ExpectedPath $standalonePath
...
if ($iscc) {
    ...
    Invoke-Checked $iscc "/DMyAppVersion=$version" "/DProjectRoot=$projectRoot" (Join-Path $projectRoot "packaging\windows\SSP-Simple-Sidechain.iss")
